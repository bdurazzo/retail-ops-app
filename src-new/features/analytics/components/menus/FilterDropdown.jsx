...
export default function FilterDropdown({
  ...
  // Expose imperative controls to parent (apply/reset)
  const applyFromOutside = async () => {
    try {
      let applied = null;
  ...
  useEffect(() => {
    if (typeof expose === 'function') {
      expose({ apply: applyFromOutside, reset: resetFromOutside });
    }
  }, [expose]);
          ...
          > 
            ...
            <div ref={contentRef}>
              ...
              */}
              ...
              {!hideFooter && (
                <div className="border-t shadow border-gray-200 px-4 py-2 flex justify-end bg-gradient-b from-white to-gray-50 sticky bottom-0 gap-3">
                  <button
                  ...
                  <button
                    className="bg-gray-900 text-white px-3 py-1 text-xs rounded-md hover:bg-gray-800"
                    onClick={applyFromOutside}
                  >
                    Apply
                  </button>
                </div>
