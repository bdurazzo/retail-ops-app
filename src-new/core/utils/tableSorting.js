...
/**
 * Toggle sort direction helper with 3-state cycle: desc -> asc -> unsorted
...
export function getNextSortState(currentKey, newKey, currentDirection) {
  if (currentKey !== newKey) {
    // Clicking a new column - default to descending
    return { sortKey: newKey, sortDirection: 'desc' };
  } else {
    // Clicking the same column - cycle through: desc -> asc -> unsorted
    if (currentDirection === 'desc') {
      return { sortKey: newKey, sortDirection: 'asc' };
    } else if (currentDirection === 'asc') {
      return { sortKey: null, sortDirection: null }; // Unsorted state
    } else {
      return { sortKey: newKey, sortDirection: 'desc' }; // Back to descending
    }
  }
}
