...
export default function GroupTable({ 
  ...
  const productConfig = useMemo(() => {
    ...
    console.log('🔍 ProductGroup: Table rows count:', table?.rows?.length);
    console.log('🔍 ProductGroup: Product filter:', productFilter);
    console.log('🔍 ProductGroup: Selected products:', selectedProducts);
    ...
    // Use the table data as-is since Discovery already filtered it to show all historical data for selected products
    let filteredTable = table;
    
    console.log('🔍 ProductGroup: Using pre-filtered table with', filteredTable.rows.length, 'rows')
  ...
  // If no products detected, show simple message
...
function ProductSection({ 
  ...
  const rightContent = (
    <div className="flex items-center mt-1 gap-2">
      ...
      <ToolbarPlugin 
        pluginSlots={{
          inserts: [], // Available for insert plugin drops
          sends: [] // Available for send plugin drops
        }}
        pluginContext={{
        ...
        onPluginData={(data) => {
          console.log(`🔌 Product "${product.name}" plugin data:`, data);
        }}
      />
    </div>
  );
  ...
  return (
    <div className="bg-white border-b border-gray-300" style={{ borderRadius: 0 }}>
      {/* Product Toolbar */}
      <Toolbar 
