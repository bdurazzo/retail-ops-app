...
import { IconShirtFilled, IconTagsFilled, IconClockHour9Filled } from '@tabler/icons-react';
...
export default function ProductPluginPanel({
  onPanelStateChange,
  panelCommand,
  // Container props
  ...
  headerText = "Product Configuration",
  // Slot wrapper props
  ...
  const getSlot1Items = () => {
    return Object.keys(slotHierarchy.selection).map(key => {
      const item = slotHierarchy.selection[key];
      return {
        value: key,
        label: item.label,
  ...
  const getSlot2Items = (slot1Value) => {
    const slot1Data = slotHierarchy.selection[slot1Value];
  ...
  const productSlots = [
    ...
    },
    {
      ...
      icon: IconTagsFilled,
      label: 'Subset',
      items: getSlot2Items(selectedParams.selection),
      footerBefore: '',
      footerAfter: ' over '
    },
    {
      ...
      label: 'Time',
      items: getSlot3Items(selectedParams.selection, selectedParams.subset),
      footerBefore: '',
      footerAfter: ''
    }
  ];
  ...
  const handleValueChange = (key, value) => {
    setSelectedParams(prev => {
      ...
      if (key === 'selection') {
        ...
        const isValidSubset = newSlot2Items.some(item => item.value === prev.subset);
        if (!isValidSubset && newSlot2Items.length > 0) {
          newParams.subset = newSlot2Items[0].value;
        }

        // Check if current slot 3 value is still valid
        const newSlot3Items = getSlot3Items(value, newParams.subset);
        ...
        // Check if current slot 3 value is still valid
        const newSlot3Items = getSlot3Items(prev.selection, value);
        const isValidTimeframe = newSlot3Items.some(item => item.value === prev.timeframe);
        if (!isValidTimeframe && newSlot3Items.length > 0) {
          newParams.timeframe = newSlot3Items[0].value;
        }
  ...
  return (
    <div className={containerClassName}>
      <div className={headerClassName}>
        <IconShirtFilled size={16} className="mr-2" />
        {headerText}
      </div>
